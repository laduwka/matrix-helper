...
- **Поиск упоминаний** - найти комнаты, в которых вас упоминали, с прямыми ссылками на Element и веб-интерфейс
- **Отметить все как прочитанное** - снять статус непрочитанных со всех комнат одной командой
- **Интерактивный режим** - ввод учётных данных через терминал с маскировкой пароля
- **Надёжная работа с API** - встроенный retry с exponential backoff, circuit breaker и rate limiter
...
Программа автоматически ограничивает частоту запросов к серверу Matrix (20 запросов/мин). При получении ошибки 429 (Too Many Requests) повторная попытка выполняется после задержки, указанной сервером.
